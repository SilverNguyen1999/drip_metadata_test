...
impl From<CsvRecord> for Metadata {
    fn from(record: CsvRecord) -> Self {
        ...
        if !record.year.trim().is_empty() {
            let year_value = if let Ok(year_num) = record.year.trim().parse::<i64>() {
                Value::Number(serde_json::Number::from(year_num))
            } else {
                Value::String(record.year.trim().to_string())
            };
            attributes.push(Attribute {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file = File::open("Ronin_Drip_Inventory__NFT (COPY) (DONT MINT THIS YET) - Sheet1.csv")?;
    ...
    for result in rdr.deserialize() {
        ...
        // Write to file named as count (0, 1, 2, etc.)
    ...
    println!("Successfully generated {} metadata files!", count);
    Ok(())
}
